...
If you discover a security issue in cloud-itonami-isic-1080, **do not open a
...
This repository implements the **Governor** for prepared-animal-feed
(mixing and pelletizing) plant operations — a deterministic safety-checking
...
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:conditioning-temp-below-minimum`,
   `:mycotoxin-level-exceeds-max`, `:foreign-material-detected`,
   `:medicated-feed-cross-contact`) blocks the proposal unconditionally
...
- Include documentation of the feed-safety rationale
